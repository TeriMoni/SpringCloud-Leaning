...
public class DateUtil {
	 ...
	 */
	 ...
	 /**
	  * 判断传入日期是否早于当前日期前几分钟
	  * @param hour,date1,format
	  * @return true:小于当前日期几分钟
	  */
	 public static boolean checkDateBeforeTime(int min,String date1,String format) {
		 boolean flag = false;
		 Calendar calendar = Calendar.getInstance();
		 calendar.set(Calendar.MINUTE, calendar.get(Calendar.MINUTE) - min);
		 SimpleDateFormat df = new SimpleDateFormat(format);
		 String beforeTime = df.format(calendar.getTime());
		 try{
			 Date lastLoginTime = df.parse(date1);
			 Date beforeLoginTime = df.parse(beforeTime);
			 if(lastLoginTime.before(beforeLoginTime)){
				 flag = true;
			 }
		 }catch(ParseException e){
			 e.printStackTrace();
	 ...
	 public static List<String> getDatesBetweenTwoDate(String startTime,
